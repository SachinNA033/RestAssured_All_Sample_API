...
import pojo2.LoginRequest;
import pojo2.LoginResponse;
import pojo2.OrderDetail;
import pojo2.Orders;
...
//SSL
		...
		RequestSpecification reqLogin = given().relaxedHTTPSValidation().log().all().spec(req).body(loginRequest); // handling SSL certification using this method relaxedHTTPSValidation()
		...
		RequestSpecification reqAddProduct = given().log().all().spec(addProductBaseReq).param("productName", "Laptop")
				...
				.multiPart("productImage", new File("C:\\Users\\SachinNagarajAchari\\Downloads\\Laptop.png"));
